    ...
    <Dialog>
      ...
      <DialogContent className="track-dialog">
        ...
        <div className="track-dialog-copy">
          ...
          {track.sunoId ? <><SunoPlayer track={track} />{track.spotifyTrackId && <a className="text-link" href={`https://open.spotify.com/track/${track.spotifyTrackId}?utm_source=p34nuts`} target="_blank" rel="noreferrer">IN SPOTIFY ÖFFNEN <ArrowUpRight size={14} /></a>}</> : <button type="button" className="track-pending-link" onClick={() => onListenRequest(track)}><Play size={14} fill="currentColor" /> STREAMING / QUELLE AUSSTEHEND</button>}
        </div>
      </DialogContent>
    </Dialog>
  );
