...
export default function Admin() {
  ...
  const [settingDrafts, setSettingDrafts] = useState<Partial<Record<SettingKey, string>>>({});
  const guestbook = trpc.guestbook.adminList.useQuery(undefined, { enabled: user?.role === "admin", staleTime: 30_000, refetchOnWindowFocus: false });
  const settings = trpc.settings.adminList.useQuery(undefined, { enabled: user?.role === "admin", staleTime: 30_000, refetchOnWindowFocus: false });
  const utils = trpc.useUtils();
  const moderate = trpc.guestbook.moderate.useMutation({ onSuccess: () => { utils.guestbook.adminList.invalidate(); utils.guestbook.list.invalidate(); setNotice("Der Gästebuchstatus wurde gespeichert."); }, onError: error => setNotice(`Gästebuchänderung fehlgeschlagen: ${error.message}`) });
  const remove = trpc.guestbook.delete.useMutation({ onSuccess: () => { utils.guestbook.adminList.invalidate(); setNotice("Der Eintrag wurde dauerhaft gelöscht."); }, onError: error => setNotice(`Löschen fehlgeschlagen: ${error.message}`) });
  const updateSetting = trpc.settings.adminUpdate.useMutation({ onSuccess: () => { utils.settings.adminList.invalidate(); setNotice("Die Einstellung wurde gespeichert."); }, onError: error => setNotice(`Speichern fehlgeschlagen: ${error.message}`) });
  ...
  useEffect(() => { if (settings.data) setSettingDrafts(Object.fromEntries(settings.data.map(item => [item.key, item.value])) as Partial<Record<SettingKey, string>>); }, [settings.data]);
  ...
  const settingValue = (key: SettingKey, fallback = "") => settingDrafts[key] ?? settingMap[key] ?? fallback;
  const saveSetting = (key: SettingKey, value = settingValue(key)) => { updateSetting.mutate({ key, value }); };
